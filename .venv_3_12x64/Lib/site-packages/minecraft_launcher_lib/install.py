...
def install_assets(data: ClientJson, path: str, callback: CallbackDict) -> None:
    ...
    def download_asset(hash: str) -> int:
        session = requests.session()
        download_file("https://resources.download.minecraft.net/" + hash[:2] + "/" + hash, os.path.join(path, "assets", "objects", hash[:2], hash), callback, sha1=hash, session=session, minecraft_directory=path)
        return 1

    hash_set = set(val["hash"] for val in assets_data["objects"].values())

    count = 0
    # Use a ThreadPoolExecutor to download assets concurrently
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        futures = [executor.submit(download_asset, hash) for hash in hash_set]
        for future in futures:
            count += future.result()
            callback.get("setProgress", empty)(count)
